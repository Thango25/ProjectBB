...
namespace Project.Controllers
{
    public class ItemsController : Controller
    {
        ...
        //  BET MODIFIED 18/10/2025: GetLatestItems - Added int? limit parameter
        ...
        public async Task<IActionResult> GetLatestItems(
            ...
            [FromQuery] int? limit) // <--- NEW PARAMETER

        {
...

            {
                ...
                query = query.Where(i => i.DateLost >= startDate.Value.Date);
...

            {
                ...
                query = query.Where(i => i.DateLost <= endDate.Value.Date.AddDays(1).AddSeconds(-1));

            }



            // Execute the query, using the dynamic limit

            var latestItems = await query

                .OrderByDescending(i => i.DateLost)

                .Take(itemLimit) // <--- USED DYNAMIC LIMIT HERE

                .Select(i => new LatestItem
